...
namespace WanPHP\Core\Repositories\WeiXin;
...
class WeChatPay
{
  ...
  {
    if (getenv('WECHAT_PAY_MERCHANT_ID') &&
      getenv('WECHAT_PAY_MERCHANT_CERTIFICATE_SERIAL') &&
      getenv('WECHAT_PAY_MERCHANT_PRIVATE_KEY_FILEPATH') &&
      getenv('WECHAT_PAY_PLATFORM_CERTIFICATE_SERIAL') &&
      getenv('WECHAT_PAY_PLATFORM_CERTIFICATE_FILEPATH') &&
      getenv('WECHAT_PAY_PLATFORM_PUBLIC_KEY_ID') &&
      getenv('WECHAT_PAY_PLATFORM_PUBLIC_KEY_FILEPATH')
    ) {
      $this->mchid = getenv('WECHAT_PAY_MERCHANT_ID');
      $this->appid = getenv('WECHAT_PAY_APPID');
      $this->notify_url = getenv('WECHAT_PAY_NOTIFY_URL');
      $this->apiV3Key = getenv('WECHAT_PAY_API_V3_KEY');
      $this->config = [
        'mchid' => $this->mchid,
        'serial' => getenv('WECHAT_PAY_MERCHANT_CERTIFICATE_SERIAL'),
        'privateKey' => Rsa::from(getenv('WECHAT_PAY_MERCHANT_PRIVATE_KEY_FILEPATH')),
        'certs' => [
          getenv('WECHAT_PAY_PLATFORM_CERTIFICATE_SERIAL') => Rsa::from(getenv('WECHAT_PAY_PLATFORM_CERTIFICATE_FILEPATH'), Rsa::KEY_TYPE_PUBLIC),
          getenv('WECHAT_PAY_PLATFORM_PUBLIC_KEY_ID') => Rsa::from(getenv('WECHAT_PAY_PLATFORM_PUBLIC_KEY_FILEPATH'), Rsa::KEY_TYPE_PUBLIC),
        ]
      ];
      // 构造一个 APIv3 客户端实例
