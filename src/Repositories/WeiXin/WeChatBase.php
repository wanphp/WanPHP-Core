...
use WanPHP\Core\Repositories\WeiXin\Traits\ImageOcrTrait;
use WanPHP\Core\Repositories\WeiXin\Traits\OfficialAccountTrait;
...
class WeChatBase
{
  ...
  public function __construct(RedisCacheFactory $cacheFactory)
  {
    $this->message_token = getenv('WECHAT_OFFICIAL_ACCOUNT_MESSAGE_TOKEN') ?: '';
    $this->appid = getenv('WECHAT_OFFICIAL_ACCOUNT_APPID') ?: '';
    $this->message_encodingAesKey = getenv('WECHAT_OFFICIAL_ACCOUNT_MESSAGE_ENCODING_AES_KEY') ?: '';
    $this->app_secret = getenv('WECHAT_OFFICIAL_ACCOUNT_APP_SECRET') ?: '';
    $this->uin_base64 = getenv('WECHAT_OFFICIAL_ACCOUNT_BASE64UID') ?: '';
    $this->webAuthorization = getenv('WECHAT_OFFICIAL_ACCOUNT_USER_AUTH') === 'true';

    $this->cache = new Psr16Cache($cacheFactory->create(0, $this->appid));
