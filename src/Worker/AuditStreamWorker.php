...
final class AuditStreamWorker
{
      ...
      try {
        ...
        foreach ($messages[$this->stream] as $id => $data) {
          try {
            ...
            error_log("[AUDIT_STREAM] invalid JSON {$id}");
          }
        }
  ...
  private function recoverPending(): void
  {
    ...
    foreach ($pending as $msg) {
      ...
      foreach ($messages[$this->stream] as $id => $data) {
        ...
        if ($this->safeInsert([$payload])) { // 统一走 safeInsert
  ...
  private function initGroup(): void
  {
    try {
      ...
      if (str_contains($e->getMessage(), 'BUSYGROUP')) {
        error_log('GROUP ALREADY EXISTS');
        return;
      }
  ...
  private function reconnectDatabase(): void
  {
    try {
      ...
      error_log('[AUDIT] MySQL Reconnected');

    } catch (\Throwable $e) {
      error_log('[AUDIT] Reconnect Failed: ' . $e->getMessage());
      sleep(2);
    }
  }
}